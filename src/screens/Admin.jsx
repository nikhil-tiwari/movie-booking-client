            ...
            (value) =>
              value.createdBy === userProfile?.id && (
                <button
                  ...
                  className="w-full mb-4 p-2 bg-blue-600 hover:bg-blue-700 rounded-xl text-center"
                >
                  {value.name}
                </button>
              )
          )}
          <button className="w-full mt-4 p-2 bg-green-600 hover:bg-green-700 rounded-xl">
            Add New Theatre
          </button>
        </div>
              ...
              <div>
                ...
                {currentTheatre.movieListed.length > 0 ? (
                  currentTheatre.movieListed.map((movieId, index) => (
                    <div key={index+1} className="mb-2 p-2 bg-gray-700 rounded">
                      <a href="">{getMovieTitleById(movieId)}</a>
                    </div>
                  ))
                ) : (
